...
async def create_shipment(
    ...
    shipment = Shipment(
        ...
        cargo_quantity=payload.get("cargo_quantity", ""),
        cbm=payload.get("cbm") or None,
        weight_kg=int(payload.get("weight_kg") or 0),
        tare_weight_kg=int(payload["tare_weight_kg"]) if payload.get("tare_weight_kg") else None,
        weight_method=payload.get("weight_method", ""),
        vgm_weight=int(payload["vgm_weight"]) if payload.get("vgm_weight") else None,
        pickup_date=_parse_dt(payload.get("pickup_date")),
        pickup_depot=payload.get("pickup_depot", ""),
        full_in_date=_parse_dt(payload.get("full_in_date")),
        full_return_to=payload.get("full_return_to", ""),
        vgm_cutoff_date=_parse_dt(payload.get("vgm_cutoff_date")),
        cutoff_date=_parse_dt(payload.get("cutoff_date")),
        etd=_parse_dt(payload.get("etd")),
        eta=_parse_dt(payload.get("eta")),
        estimated_on_board_date=_parse_dt(payload.get("estimated_on_board_date")),
        issue_date=_parse_dt(payload.get("issue_date")),
        stowage_code=payload.get("stowage_code", ""),
        exchange_rate=payload.get("exchange_rate") or None,
        description=payload.get("description", ""),
        supplier=payload.get("supplier", ""),
        reference=payload.get("reference", ""),
        notes=payload.get("notes", ""),
        freight_cost=payload.get("freight_cost") or 0,
        customs_cost=payload.get("customs_cost") or 0,
        other_cost=payload.get("other_cost") or 0,
...
async def update_shipment(
    ...
    updatable = [
        ...
        "place_of_receipt", "port_of_loading", "port_of_discharge", "place_of_delivery",
        "container_number", "container_type", "service_type", "cargo_quantity",
        "cbm", "weight_kg", "tare_weight_kg", "weight_method", "vgm_weight",
        "pickup_date", "pickup_depot", "full_in_date", "full_return_to",
        "vgm_cutoff_date", "cutoff_date", "etd", "eta",
        "estimated_on_board_date", "issue_date",
        "actual_departure", "actual_arrival", "status", "last_event",
        "stowage_code", "exchange_rate",
