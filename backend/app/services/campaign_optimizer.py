...
def generate_optimization_steps(
    ...
    if zero_conv_terms:
        steps.append({
            "timing": "立即",
            "action": f"否定 {len(zero_conv_terms)} 个 0 转化搜索词",
            "details": zero_conv_terms[:15],
        })
...
async def get_optimization_recommendations(
    ...
    if campaign_ids:
        ...
        for r in zc_rows:
            zero_conv.setdefault(str(r[0]), []).append(str(r[1]))
    ...
    high_conv_terms = [str(r[0]) for r in quick_win_rows if r[0]]
    ...
    for source in critical:
        ...
        zc_terms = zero_conv.get(source["campaign_id"], [])[:15]
    ...
    if high_conv_terms:
        quick_wins.append({
            ...
            "impact": "预计 ACoS 降 3-5pp，提升精准流量",
            "terms": high_conv_terms[:10],
        })
