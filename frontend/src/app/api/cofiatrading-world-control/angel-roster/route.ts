...
const ANGEL_ROSTER: Angel[] = [
  ...
  { id: 3,  name: "Isrāfīl",         name_ar: "إِسْرَافِيل",    platform: "CofiaPublisher :8540",     manzilah: "Trompette · pipeline vidéo",               status: "CANON_GATE",   mission: "Publication vidéos · 89 MP4 prêts (curl :8540/api/status output_dir_count) · publish §18", stack: "ffmpeg + Remotion local · service LIVE HTTP 200", proof_url: "curl :8540/api/status → 200" },
  ...
  { id: 8,  name: "Ridwān",          name_ar: "رِضْوَان",        platform: "VIP Welcome (iron_stripe_onboard)",     manzilah: "Onboarding VIP post-signup",  status: "LIVE",     mission: "iron-stripe-onboard LaunchAgent UP (exit 0) · KG :8435 UP HTTP 200 · poll 5min · 0 new signups 1440min lookback (vrai, pas un bug)", stack: "iron_stripe_onboard.py + KG :8435 + Resend + Telegram", proof_url: "launchctl list | grep iron-stripe-onboard exit=0 ; curl :8435 → HTTP 200" },
  ...
  { id: 20, name: "Jack",            name_ar: "جَاكْ",           platform: "Brokers CellXpert",        manzilah: "Affiliation reclaim 6884 broker_accounts", status: "BROKEN",       mission: "IP_NOT_AUTHENTICATED 15j · 6884 broker_accounts 99% Default · Slack ops message drafté /pending P1 · Erwin envoie aux affiliate managers FXcess+IronFX+Libertex", stack: "broker-tracker.sh cron 4h", proof_url: "~/.openclaw/state/pending/p1_brokers_ip_fix_20260526T1325Z/slack_message_brokers.md", arr_impact_eur_year: -200000 },
  ...
  { id: 23, name: "Quant",           name_ar: "كُوَانْتْ",        platform: "Strategy Lab",             manzilah: "Backtest + edge research",                 status: "OPERATIONAL_PARTIAL",  mission: "strategy-lab cron LIVE last exit 0 (CLAUDE.md ligne 144 outdated · vérif Sidq 2026-05-26T13:35Z)", stack: "Python local · launchctl list grep OK", proof_url: "launchctl list | grep strategy-lab → status 0" },
  ...
  { id: 31, name: "Guardian",        name_ar: "غَارْدِيَان",     platform: "Memory sync + Mu'taqib doctrine drift",   manzilah: "Anti-Alzheimer + auditeur non-bloquant",            status: "LIVE",     mission: "Préserve mémoire cross-sessions + opère mutaqib_doctrine_drift_sentinel.py (advisor warnings, JAMAIS bloquant — §51)",                              stack: "STATE_LIVE_AUTO.md cron 300s + mutaqib_doctrine_drift_sentinel.py", proof_url: "ls ~/.openclaw/scripts/sentinel/mutaqib_doctrine_drift_sentinel.py ; curl :3000/api/cofiatrading-world-control/mutaqib-warnings" },
  ...
  { id: 35, name: "Lab",             name_ar: "لَابْ",           platform: "MiroFish predictions",     manzilah: "Predictions marché expérimentales",        status: "OPERATIONAL_PARTIAL",  mission: "lab-tick + mirofish-tick + paperclip cron LIVE last exit 0 (CLAUDE.md ligne 144 outdated · vérif Sidq 2026-05-26T13:35Z) · predictions pas encore exploitées Da'wah", stack: "Python local · launchd cron périodique", proof_url: "launchctl list | grep mirofish-tick → status 0" },
