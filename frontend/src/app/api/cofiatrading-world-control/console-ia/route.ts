...
// Flux 6 — CRM 360 vivant : source canonique = iron_crm_ultra_runtime.json, indexé par
// telegram_id (clients_by_tg). Lecture seule, cache mtime (le fichier fait ~6.6 Mo).
const CRM_ULTRA_JSON = path.join(STATE_ROOT, "iron_crm", "iron_crm_ultra_runtime.json");
...
function computeRouteBuses(targetId: string, modelModeId: string): string[] {
  const routeConfig = TARGET_ROUTES[targetId] ?? TARGET_ROUTES.central_council;
...
async function approvePacket(body: Record<string, unknown>) {
  const packetId = sanitizeText(asString(body.packetId), 140);
  ...
  const packetHash = createHash("sha256").update(JSON.stringify(packet)).digest("hex");
  await writeFile(packetPath, safeJson({ ...packet, packetHash }), "utf8");
  publishCentralBrainEvent("console_ia_packet_approved", {
...
async function rejectPacket(body: Record<string, unknown>) {
  const packetId = sanitizeText(asString(body.packetId), 140);
  const approver = sanitizeText(asString(body.approver), 40) || "erwin";
  const packetPath = path.join(PACKETS_DIR, `${packetId}.json`);
  if (!packetId || !(await fileExists(packetPath))) {
  ...
  const packetHash = createHash("sha256").update(JSON.stringify(packet)).digest("hex");
  await writeFile(packetPath, safeJson({ ...packet, packetHash }), "utf8");
  publishCentralBrainEvent("console_ia_packet_rejected", {
