...
export default function H5ChatListPage() {
  ...
  return (
    <div className="flex h-full flex-col">
      <H5Header title={t("h5.sessions.title")} />

      {/* Logout button */}
      <div className="flex items-center justify-between border-b border-line px-4 py-2">
        <p className="text-sm text-muted">
          {t("h5.sessions.loggedInAs").replace("{username}", user.display_name ?? user.username)}
        </p>
        <button
          onClick={handleLogout}
          className="flex items-center gap-1 text-xs text-muted hover:text-strong"
          aria-label={t("h5.sessions.logout")}
        >
          <LogOut className="h-3.5 w-3.5" />
          {t("h5.sessions.logout")}
        </button>
      </div>
