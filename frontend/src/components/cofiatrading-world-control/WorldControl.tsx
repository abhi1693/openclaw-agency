      ...
      <main className="flex-1 overflow-y-auto bg-slate-950">
        ...
        <div className="mx-auto grid max-w-[1760px] gap-4 xl:grid-cols-[1fr_420px]">
          <div className="space-y-4">
            <header className="rounded-md border border-cyan-300/20 bg-slate-950/85 p-4 shadow-[0_0_45px_rgba(34,211,238,0.08)]">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="flex min-w-[280px] items-start gap-3">
                  <div className="flex h-11 w-11 items-center justify-center rounded-md border border-cyan-300/35 bg-cyan-300/10 text-cyan-100">
                    <ShieldCheck className="h-5 w-5" />
                  </div>
                  <div>
                    <h1 className="font-heading text-2xl font-black uppercase text-white">
                      COFIATRADING OPERATOR CONTROL
                    </h1>
                    <p className="mt-1 text-sm text-slate-300">
                      Business truth · 15 maisons · runtime snapshot
                    </p>
                    <p className="mt-1 flex items-center gap-1.5 text-[11px] font-semibold text-emerald-300">
                      ...
                      />
                      <span>LIVE · synced {secondsSinceSync}s ago · {snapshot?.fetchedAt ?? "UNKNOWN"}</span>
                    </p>
                  </div>
                </div>
                <div className="rounded-md border border-amber-300/25 bg-amber-300/10 px-3 py-2 text-right">
                  <p className="text-[11px] font-semibold uppercase text-amber-100">System truth</p>
                  <p className="mt-1 text-xl font-black text-white">{serviceStatus}</p>
                  <p className="text-[11px] text-amber-100/75">services OK · proof {endpointStatus}</p>
                </div>
              </div>

              <div className="mt-4 grid gap-2 md:grid-cols-4 xl:grid-cols-7">
                <TopMetric label="ARR" value={formatEur(snapshot?.revenue.currentArrEur)} isPulsing={pulsingFields.has("arr")} />
                <TopMetric label="MRR" value={formatEur(snapshot?.revenue.currentMrrEur)} isPulsing={pulsingFields.has("mrr")} />
                <TopMetric label="VIP" value={formatNumber(snapshot?.revenue.activeVip)} isPulsing={pulsingFields.has("vip")} />
                <TopMetric label="Past due" value={`${formatEur(snapshot?.revenue.pastDueEur)} / ${formatNumber(snapshot?.revenue.pastDueCount)}`} isPulsing={pulsingFields.has("pastDue")} />
                <TopMetric label="Maisons" value={formatNumber(snapshot?.centralBrain.housesCount)} />
                <TopMetric label="Services" value={serviceStatus} />
                <TopMetric label="Assets" value={assetsProof} />
              </div>
            </header>

            <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
              <Panel title="3 actions prioritaires" tone="gold">
                <div className="grid gap-2">
                  {primaryActions.length > 0 ? (
                    primaryActions.map((action, index) => (
                      <div key={`${action.title}-${index}`} className="rounded-md border border-amber-300/25 bg-amber-300/10 p-3">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className={`shrink-0 rounded border px-2 py-0.5 text-[10px] font-semibold ${statusClass[normalizeStatus(action.priority)]}`}>
                            {action.priority || "UNKNOWN"}
                          </span>
                        </div>
                        <div className="mt-2 grid gap-2 text-[11px] text-slate-300 sm:grid-cols-2">
                          <span>ARR impact: {action.arr_impact || "UNKNOWN"}</span>
                          <span className="truncate">source_tag: {action.source_tag || "UNKNOWN"}</span>
                        </div>
                      </div>
                    ))
                  ) : (
                    <div className="rounded-md border border-slate-700 bg-slate-950/70 p-3 text-sm text-slate-300">
                      UNKNOWN until investor_room.next_7_days_tasks returns.
                    </div>
                  )}
                </div>
              </Panel>
              ...
              <Panel title="15 maisons habitées" tone="cyan">
                <div className="mb-3 rounded-md border border-emerald-300/20 bg-emerald-300/10 px-3 py-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div>
                      <p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-100">
                        Workforce active — 38 anges organisés par maison
                      </p>
                      <p className="mt-1 text-[11px] text-emerald-100/75">
                        {mappedWorkers.length} ouvriers affectés · {rosterRuntimeLabel}
                      </p>
                    </div>
                    <p className="text-[11px] text-emerald-100/75">
                      Top équipes: {topWorkforceHouses.map((house) => `${house.displayName} ${house.workforce.workers.length}`).join(" · ")}
                    </p>
                  </div>
                </div>
                <div className="grid gap-2 lg:grid-cols-2 2xl:grid-cols-3">
                  {visibleHouseCards.map((house) => {
                    const workforce = house.workforce;
                    const visibleWorkers = workforce.workers.slice(0, 5);
                    const hiddenWorkersCount = Math.max(0, workforce.workers.length - visibleWorkers.length);
                    ...
                    const trucksLabel = house.trucks.length > 0
                      ? `Camions: ${house.trucks.length} prouvés runtime`
                    ...
                    return (
                      <button
                        ...
                        className="min-w-0 rounded-md border border-slate-800 bg-slate-950/75 px-3 py-2 text-left transition hover:border-cyan-300/50 hover:bg-cyan-300/10"
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            <p className="truncate text-sm font-black text-slate-100">{house.displayName}</p>
                            <p className="mt-0.5 truncate text-[10px] text-cyan-200/70">{house.id}</p>
                          </div>
                          <span className={`shrink-0 rounded border px-1.5 py-0.5 text-[9px] font-semibold ${statusClass[runtimeTone]}`}>
                            {runtimeBadge}
                          </span>
                        </div>
                        <div className="mt-2 grid gap-1 text-[10px] leading-4 text-slate-300">
                          <p><span className="text-slate-500">Owner:</span> {workforce.owner}</p>
                          <p className="line-clamp-2"><span className="text-slate-500">Mission:</span> {workforce.mission}</p>
                          <p className="line-clamp-2"><span className="text-slate-500">Next:</span> {workforce.nextAction}</p>
                          <p className="line-clamp-2"><span className="text-slate-500">Blocage:</span> {workforce.blocker}</p>
                          <p><span className="text-slate-500">Impact:</span> {workforce.impact}</p>
                          <p className="line-clamp-2"><span className="text-slate-500">Preuve:</span> {workforce.proof}</p>
                          <p className="text-slate-400">{trucksLabel}</p>
                        </div>
                        <div className="mt-2 flex flex-wrap gap-1">
                          {visibleWorkers.map((worker) => {
                            const rosterStatus = rosterStatusByName.get(worker.toLowerCase());
                            return (
                              <span
                                key={`${house.id}-${worker}`}
                                className="rounded border border-cyan-300/25 bg-cyan-300/10 px-1.5 py-0.5 text-[9px] font-semibold text-cyan-100"
                                title={rosterStatus ? `${worker} · runtime ${rosterStatus}` : `${worker} · affectation organisationnelle`}
                              >
                                {worker}
                              </span>
                            );
                          ...
                          {hiddenWorkersCount > 0 ? (
                            <span className="rounded border border-slate-600 bg-slate-800/60 px-1.5 py-0.5 text-[9px] font-semibold text-slate-200">
                              +{hiddenWorkersCount}
                            </span>
                          ) : null}
                        </div>
            ...
            <Panel title="System truth" tone="cyan">
              <div className="grid gap-2 md:grid-cols-4">
                {services.length > 0 ? services.map((service) => (
                  <div key={service.id} className="rounded-md border border-slate-800 bg-slate-950/70 px-3 py-2">
                    <div className="flex items-center justify-between gap-2">
                      <p className="truncate text-xs font-semibold text-white">{service.label}</p>
                      <span className={`shrink-0 rounded border px-2 py-0.5 text-[9px] font-semibold ${statusClass[service.ok ? "LIVE" : "AMBER"]}`}>
                        {service.ok ? "OK" : "CHECK"}
                      </span>
                    </div>
                    <p className="mt-1 truncate text-[10px] text-slate-500">
                      HTTP {service.http_code ?? service.status ?? "UNKNOWN"} · {service.role ?? service.id}
                    </p>
                  </div>
                )) : (
                  <div className="rounded-md border border-slate-800 bg-slate-950/70 p-3 text-sm text-slate-300">
                    UNKNOWN until snapshot.services returns.
                  </div>
                )}
              </div>
            </Panel>
          ...
          <aside className="space-y-4">
            <Panel title="Assets warehouse" tone="gold">
              <MetricGrid
                metrics={[
                  ["MP4", formatNumber(snapshot?.assetsWarehouse?.mp4Count)],
                  ["Captions", formatNumber(snapshot?.assetsWarehouse?.captionsCount)],
                  ["Assets", formatNumber(snapshot?.assetsWarehouse?.assetsInventoryCount)],
                  ["Publisher", formatNumber(snapshot?.publisher.outputDirCount)],
                ]}
              />
              <p className="mt-2 text-xs text-amber-100/75">
                {snapshot?.assetsWarehouse?.ok ? "LIVE" : "UNKNOWN"} · {snapshot?.assetsWarehouse?.sourceTag ?? "UNKNOWN"}
              </p>
            </Panel>

            <Panel title="Vision Map — secondaire" tone="cyan">
              <button
                type="button"
                onClick={openCastleDrawer}
                className="group block w-full overflow-hidden rounded-md border border-slate-700 bg-slate-950 text-left transition hover:border-amber-200/60"
              >
                <img
                  src={northStarImage}
                  alt="COFIATRADING New York Vision Map secondaire"
                  className="h-[190px] w-full object-cover opacity-70 transition group-hover:opacity-85"
                />
                <div className="border-t border-slate-800 px-3 py-2">
                  <p className="text-xs font-semibold text-slate-100">Poster Manhattan réduit en carte secondaire.</p>
                  <p className="mt-1 text-[11px] text-slate-500">
                    100M ARR map · {TARGET_DATE} · vision, pas cockpit principal.
                  </p>
                </div>
              </button>
            </Panel>

            <Panel title="Runtime locks" tone="locked">
              <ProofRow label="Old city" status="LOCKED" proof="Abidjan read-only; no old wall enters New York." />
              <ProofRow label="Writes" status="LOCKED" proof="send/publish/deploy/Stripe disabled from this cockpit." />
              <ProofRow label="Investor room" status={investorRoom ? "LIVE" : "UNKNOWN"} proof={`${formatEur(snapshot?.revenue.currentArrEur)} ARR · gap ${formatEur(arrGap)}`} />
            </Panel>
          </aside>
        </div>
      </section>
          ...
          <Panel title="15 maisons SSOT peuplées" tone="cyan">
            <div className="grid gap-2 sm:grid-cols-2">
              {houses.map((house) => {
                ...
                const runtimeBadge = house.status === "LIVE" ? "LIVE" : workforce.badge;
                const trucksLabel = house.trucks.length > 0
                  ? `camions runtime: ${house.trucks.length}`
                  : "camions: source non câblée";
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="truncate text-sm font-semibold text-slate-100">{workforce.businessName}</span>
                      <span className={`rounded border px-2 py-0.5 text-[10px] ${statusClass[runtimeTone]}`}>
                        {runtimeBadge}
                      </span>
                    </div>
                    <p className="mt-1 truncate text-[11px] text-slate-500">{house.id}</p>
                    <p className="mt-2 text-[11px] text-slate-300">
                      Owner: {workforce.owner} · Ouvriers: {workforce.workers.slice(0, 4).join(", ")}
                      {workforce.workers.length > 4 ? ` +${workforce.workers.length - 4}` : ""}
                    </p>
                    <p className="mt-1 text-[11px] text-slate-400">
                      {trucksLabel} · Next: {truncateText(workforce.nextAction, 58)}
                    </p>
                  </button>
                );
              })}
...
function CalendarPanel() {
  ...
  return (
    <section className="mt-4 rounded-xl border border-cyan-300/20 bg-slate-950/70 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-[12px] font-black uppercase tracking-[0.2em] text-cyan-100">
          Calendrier opérations · cadence live
        </h2>
        <span className="text-[10px] text-slate-400">
          {err
            ? "source down"
            : `${data?.freshness ?? "…"} · ${data?.totalLines ?? "—"} events${
                typeof data?.ageS === "number" ? ` · ${data.ageS}s` : ""
              }`}
        </span>
      </div>
      <p className="mt-1 text-[10px] text-slate-500">
        Source réelle : hub :8430 /api/calendar/upcoming (calendar_bus.jsonl)
      </p>
      <div className="mt-3 space-y-1.5">
        {events.length === 0 && !err && <p className="text-[11px] text-slate-500">chargement…</p>}
        {events.length === 0 && err && (
          <p className="text-[11px] text-rose-300">source down — aucun événement affiché (honnête, pas de fake)</p>
        )}
        {events.slice(0, 12).map((e, i) => (
          <div
            key={i}
            className="flex items-center gap-2 rounded-md border border-slate-800 bg-slate-950/60 px-2.5 py-1.5 text-[11px]"
          >
