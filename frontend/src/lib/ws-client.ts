...
/**
 * WebSocket client for the H5 chat relay service (M4).
 *
 * Features:
 * - Exponential backoff reconnect (1s → 2s → 4s → 8s → 16s, max 30s)
 * - Client-side heartbeat every 30s
 * - Per-type message handler registration
 */
export class WSClient {
  ...
  sendChat(
    ...
    this.send(
      "chat",
      { agent_id: agentId, content, session_id: sessionId },
  ...
  private openSocket(): void {
    this.setStatus(
      this.backoff.attempt() === 0 ? "connecting" : "reconnecting",
    );
  ...
  private startHeartbeat(): void {
    ...
    this.heartbeatTimer = setInterval(() => {
      this.send("heartbeat", {}, crypto.randomUUID());
